...
void AnticheatMgr::JumpHackDetection(Player* player, MovementInfo /* movementInfo */,uint32 opcode)
{
    ...
    if (m_Players[key].GetLastOpcode() == MSG_MOVE_JUMP && opcode == MSG_MOVE_JUMP)
    {
		...
		sLog->outDebug(LOG_FILTER_PLAYER, "AnticheatMgr:: Jump-Hack detected player GUID (low) %u", player->GetGUIDLow());
    }
}
...
void AnticheatMgr::WalkOnWaterHackDetection(Player* player, MovementInfo /* movementInfo */)
{
	...
	sLog->outDebug(LOG_FILTER_PLAYER, "AnticheatMgr:: Walk on Water - Hack detected player GUID (low) %u", player->GetGUIDLow());
...
void AnticheatMgr::FlyHackDetection(Player* player, MovementInfo /* movementInfo */)
{
	...
	sLog->outDebug(LOG_FILTER_PLAYER, "AnticheatMgr:: Fly-Hack detected player GUID (low) %u", player->GetGUIDLow());
...
void AnticheatMgr::TeleportPlaneHackDetection(Player* player, MovementInfo movementInfo)
{
    ...
    if (z_diff > 1.0f)
    {
		sLog->outDebug(LOG_FILTER_PLAYER, "AnticheatMgr:: Teleport To Plane - Hack detected player GUID (low) %u", player->GetGUIDLow());
...
void AnticheatMgr::ClimbHackDetection(Player *player, MovementInfo movementInfo, uint32 opcode)
{
    ...
    {
		sLog->outDebug(LOG_FILTER_PLAYER, "AnticheatMgr:: Climb-Hack detected player GUID (low) %u", player->GetGUIDLow());
...
void AnticheatMgr::SpeedHackDetection(Player* player,MovementInfo movementInfo)
{
	...
	//uint32 speedRate = (uint32)(player->GetSpeed(UnitMoveType(moveType)) + movementInfo.height.xyspeed);
    ...
    if (clientSpeedRate > speedRate)
    {
		...
		sLog->outDebug(LOG_FILTER_PLAYER, "AnticheatMgr:: Speed-Hack detected player GUID (low) %u", player->GetGUIDLow());
    }
}
...
void AnticheatMgr::BuildReport(Player* player,uint8 reportType)
{
    ...
    if (m_Players[key].GetTotalReports() > sWorld->getIntConfig(CONFIG_ANTICHEAT_REPORTS_INGAME_NOTIFICATION))
    {
        ...
        str = "[PPrince AntiCheat System]|cFF00FFFF[|cFF60FF00" + std::string(player->GetName().c_str()) + "|cFF00FFFF] Possible cheater!";
