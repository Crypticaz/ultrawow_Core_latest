    ...
    public:
        ...
        class spell_warr_bloodthirst_heal_SpellScript : public SpellScript
        {
            ...
            void HandleHeal(SpellEffIndex /*effIndex*/)
            {
                ...
                if (!GetHitUnit())
                    return;
                int amount = GetCaster()->CountPctFromMaxHealth(1);
				amount = GetCaster()->SpellHealingBonusDone(GetHitUnit(), GetSpellInfo(), amount, HEAL);
				amount = GetHitUnit()->SpellHealingBonusTaken(GetSpellInfo(), amount, HEAL);
                
                // Glyph of Bloody Healing
                if (GetHitUnit()->HasAura(58369))
                    amount *= 1.4f;
