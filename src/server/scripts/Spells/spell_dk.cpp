...
class spell_dk_death_strike_enabler : public SpellScriptLoader
{
    public:
        ...
        class spell_dk_death_strike_enabler_AuraScript : public AuraScript
        {
            ...
            void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
            {
                DamagePerSecond[index] += eventInfo.GetDamageInfo()->GetDamage();
            }
            ...
            void Update(AuraEffect const* aurEff)
            {
                // Actualizamos el valor para el siguiente segundo
                if (++index >= 5)
                    index = 0;
                // Reiniciamos el valor para ese segundo
                DamagePerSecond[index] = 0;
            }
            ...
            void HandleCalcAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& canBeRecalculated)
            {
                ...
                amount = 0;
                // Calcula el damage de los 5 segundos previos
                for (uint8 i = 0; i < 5; ++i)
                    amount += int32(DamagePerSecond[i]);
            }
            ...
            void Register() 
            {
                ...
                // Aumenta el damage provocado durante el segundo actual
                OnEffectProc += AuraEffectProcFn(spell_dk_death_strike_enabler_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
                // Establece el periodo en un segundo para calcular el damage por segundo
                ...
                // Cambia de segundo activo para el calculo de damage
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_dk_death_strike_enabler_AuraScript::Update, EFFECT_0, SPELL_AURA_DUMMY);
                // Calcula la cantidad
