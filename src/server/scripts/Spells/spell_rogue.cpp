        ...
        class spell_rog_rupture_AuraScript : public AuraScript
        {
            ...
            void CalculateAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& canBeRecalculated)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    if (GetUnitOwner())
                    {
                        if (Aura* revelador = GetUnitOwner()->GetAura(84617, caster->GetGUID()))
                        {
                            AddPct(amount, 35);
                            revelador->Remove();
                        }
...
public:
	...
	class spell_rog_backstab_SpellScript : public SpellScript
	{
		...
		void HandleAfterHit()
		{
			...
			{
				int32 bp = 0;
				if (m_caster->HasAura(14158))
					bp = 15;
				if (m_caster->HasAura(14159))
					bp = 30;
				if (bp != 0)
					m_caster->CastCustomSpell(m_caster, 79132, &bp, NULL, NULL, true);
			}
		}
    ...
    public:
        ...
        class spell_rog_gouge_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckCast()
            {
                ...
                if (caster && target)
                {
                    if (!target->HasInArc(static_cast<float>(M_PI),caster)) // estamos detras del objetivo
                    {
                        if (caster->HasAura(56809)) // glifo de gubia permite usarlo desde detras
                            return SPELL_CAST_OK;
                        return SPELL_FAILED_NOT_INFRONT;
                    }
                    else // esta delante, se ejecutara siempre bien
                        return SPELL_CAST_OK;
                }
                return SPELL_FAILED_NOT_INFRONT;
            }
            ...
            void HandleAfterHit()
            {
                if (GetCaster()->GetTypeId() == TYPEID_PLAYER)
                    if (Player* player = GetCaster()->ToPlayer())
                        if (Unit* target = GetHitUnit())
                            player->CastSpell(target, 76582, true);
            }

            void Register()
            {
                OnCheckCast += SpellCheckCastFn(spell_rog_gouge_SpellScript::CheckCast);
                AfterHit += SpellHitFn(spell_rog_gouge_SpellScript::HandleAfterHit);
            }
        };
...
public:
    spell_rog_deadly_brew()
        : SpellScriptLoader("spell_rog_deadly_brew")
    {
    }
...
public:
    spell_rog_deadly_momentum()
        : SpellScriptLoader("spell_rog_deadly_momentum")
    {
    }
...
public:
    spell_rog_eviscerate()
        : SpellScriptLoader("spell_rog_eviscerate")
    {
    }
